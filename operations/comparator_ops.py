...
COMPARATOR_OPS = {
    ...
    ast.Is: ComparatorOp(
        evaluate=lambda env, eval_fn, a, b: eval_fn(env, a) is eval_fn(env, b),
        pprint=lambda str_fn, a, b: str_fn(a) + " is " + str_fn(b),
    ),
    ast.IsNot: ComparatorOp(
        evaluate=lambda env, eval_fn, a, b: eval_fn(env, a) is not eval_fn(env, b),
        pprint=lambda str_fn, a, b: str_fn(a) + " is not " + str_fn(b),
    ),
    ast.In: ComparatorOp(
        evaluate=lambda env, eval_fn, a, b: eval_fn(env, a) in eval_fn(env, b),
        pprint=lambda str_fn, a, b: str_fn(a) + " in " + str_fn(b),
    ),
    ast.NotIn: ComparatorOp(
        evaluate=lambda env, eval_fn, a, b: eval_fn(env, a) not in eval_fn(env, b),
